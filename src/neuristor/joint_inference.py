...
def feature_loss(measured, predicted, *, missing_frequency_penalty=0.0):
    ...
    persistence = 0.0
    if ms["sustained"]:
        # A smooth deficit keeps small but persistent measured oscillators from
        # being traded away for a lower mean/amplitude error elsewhere.
        target = np.maximum(np.minimum(mw.periodic_vpp_mV.to_numpy(), 12.0), 6.0)
        deficit = np.maximum(1.0-pw.periodic_vpp_mV.to_numpy()/target, 0.0)
        persistence = np.mean(deficit**2) + max(1-ps["amplitude_retention"]/.5, 0)**2
        persistence += max(1-ps["late_coherence"]/.4, 0)**2
        frequency = float(missing_frequency_penalty)
    if ms["sustained"] and np.isfinite(ps["late_frequency_MHz"]):
        ...
        frequency = gate*((ps["late_frequency_MHz"]-ms["late_frequency_MHz"])/10.0)**2 + (1-gate)*missing_frequency_penalty
    return dict(mean=float(mean), amplitude=float(amp), robust=float(robust), frequency=float(frequency), persistence=float(persistence))


def combined_feature_loss(terms, settings):
    """Apply the recipe's persistence weight while retaining old-recipe scores."""
    return (terms["mean"] + 3*terms["amplitude"] + terms["robust"] + .5*terms["frequency"]
            + float(settings.get("persistence_weight", 0.0))*terms["persistence"])
...
def fit_joint(dataset, rt, base, settings, seed_models, progress=None):
    ...
    attempts = 0
    while len(population) < popsize//2 and attempts < 2048:
        attempts += 1
    ...
    def components(unit):
        ...
        if key not in cache:
            ...
            terms = dict(mean=0., amplitude=0., robust=0., frequency=0., persistence=0.)
            ...
            if not screened:
                ...
                errors = [feature_loss(m, p, missing_frequency_penalty=float(settings.get("missing_frequency_penalty", 0))) for m, p in zip(measured, features)]
                terms = {name: float(np.mean([e[name] for e in errors])) for name in terms}
            waveform = combined_feature_loss(terms, settings)
