...
\begin{enumerate}
    ...
    \begin{equation}
    T_{pr,n}\leftarrow
    \delta_n\frac{w}{2}+T_c-\frac{1}{\beta}\operatorname{arctanh}\!\left(2\,\mathrm{clip}(g_{r,n},10^{-6},1-10^{-6})-1\right)-T_{r,n},
    \quad
...
The sensitivity was associated with the finite deadband in a historical local hysteresis reversal detector. That detector updated its temperature anchor after every evaluated point, so a reversal was accepted only when the temperature change from the previous evaluated point exceeded the fixed threshold
\begin{equation}
|\Delta T_{\mathrm{step}}|>0.01~\mathrm{K}.
\label{eq:reversal_step_threshold}
\end{equation}
Changing the timestep changes \(\Delta T_{\mathrm{step}}\), and therefore can change whether and when the hysteresis branch reverses. This convention is still used only to replay saved measured R--T sample fits in the Samples tab; time-domain simulations use the accumulated-anchor detector in Eqs.~\eqref{eq:disc_reversal_assign}--\eqref{eq:disc_reversal_tpr}.

Historical control experiments also tested an accumulated-displacement detector and a turning-point extension. The turning-point extension tracks the latest temperature extremum \(T_{\mathrm{extreme}}\) and confirms a reversal after the cumulative excursion satisfies
\begin{equation}
\left|T-T_{\mathrm{extreme}}\right|>0.01~\mathrm{K}.
\label{eq:reversal_cumulative_threshold}
\end{equation}
In the extension, the reversal point is assigned to \(T_{\mathrm{extreme}}\), rather than to the sampled detection point used by the fitted app model. This can improve timestep robustness but changes minor-loop geometry. These alternate detectors are no longer selectable in the Streamlit interface.
