...
\begin{enumerate}
    ...
    \begin{equation}
    T_{pr,n}\leftarrow
    \delta_n\frac{w}{2}+T_c-\frac{1}{\beta}\operatorname{arctanh}\!\left(2g^*_{r,n}-1\right)-T_{r,n},
    \quad
    ...
    \item Set \(T^{last}_{n+1}\leftarrow T_n\) only after the deadband has been crossed. Sub-threshold motion therefore accumulates from the last accepted anchor.
\end{enumerate}

Here \(g^*_{r,n}=g_{r,n}\) for every interior float32 value. Only exact
endpoints \(g_{r,n}=0\) or \(1\) are replaced by the nearest numerically safe
open-interval values. A previous blanket clip to
\([10^{-6},1-10^{-6}]\) changed valid near-saturated minor loops and is not
used in the active time-domain model.

The Samples page also contains a fit-replay compatibility path for saved measured R--T presets. Those presets were calibrated with a point-to-point deadband convention, so the app uses that replay only for displaying their stored R--T overlays and RMSE values. Time-domain simulations use the accumulated-anchor update above.

\subsection{Discretization of the Current-Source Model}
\subsubsection*{Discrete Update Equation}
For the direct ideal-current model of Section 2, freeze \(R_n\) and
\(I_{in,n}\) over one step and define \(V_{\infty,n}=I_{in,n}R_n\). The
electrical subproblem then has the exact update
\begin{equation}
V_{n+1}
=
V_{\infty,n}+(V_n-V_{\infty,n})
\exp\!\left(-\frac{\Delta t}{R_nC}\right),
\qquad
R_n=R(T_n,\mathcal{H}_n).
\label{eq:disc_current_drive_v}
\end{equation}
For \(C=0\), the implementation uses the algebraic limit
\(V_n=I_{in,n}R_n\), so there is no electrical state or electrical time
constant.

With \(P_n=V_n^2/R_n\) and
\(T_{\infty,n}=T_0+P_n/S_e\), the frozen-power deterministic cooling
subproblem is also advanced exactly, followed by the stochastic increment:
\begin{equation}
T_{n+1}
=
T_{\infty,n}+(T_n-T_{\infty,n})
\exp\!\left(-\frac{S_e\Delta t}{C_{th}}\right)
...
\begin{enumerate}
    ...
    \item If \(C=0\), enforce \(V_n=I_{in,n}R_n\); otherwise update
    \(V_{n+1}\) with Eq.~\eqref{eq:disc_current_drive_v}.
...
\section{Supported Yuanhang Current-Source Model}

\subsection{Official current-source equations}
The supported current-source circuit is the ideal imposed-current reduction:
\begin{align}
C\frac{dV}{dt} &= I_{\mathrm{in}}-\frac{V}{R}, \label{eq:official_current}\\
C_{\mathrm{th}}\frac{dT}{dt} &= \frac{V^2}{R}-S_e(T-T_0). \label{eq:official_thermal}
\end{align}
The resistance is evaluated with the upstream-faithful Yuanhang hysteresis law:
\begin{equation}
R(T)=R_m+R_0\exp\left(\frac{E_a/k_B}{T}\right)
g_{\mathrm{eq}}\!\left(T,\mathcal{H}\right).
\label{eq:official_current_resistance}
\end{equation}
The independent dynamical states are \(V\) and \(T\). The phase fraction is not an additional dynamical state; it is the instantaneous value prescribed by the Yuanhang hysteresis state \(\mathcal{H}\). Optional stochastic heating may be enabled for exploratory noisy runs, but \(\sigma=0\) is the deterministic default.

\subsection{Removed current-source alternatives}
The codebase previously contained local current-source extensions for delayed phase response, double thermal bodies, parallel domains, and alternate reversal conventions. Those paths are now treated as nonphysical for this project and have been removed from the active simulator, scripts, and app preset folder. Their old fields are stripped when historical job JSON is loaded so old records do not crash the app, but rerunning an old current-source record uses Equations~\eqref{eq:official_current}--\eqref{eq:official_current_resistance}.

This matters because the old extensions changed the mechanism being tested. A delayed phase variable adds an extra state and an uncalibrated time constant; the double-thermal and multidomain variants add spatial or substrate degrees of freedom; and alternate reversal conventions change the minor-loop geometry. Those mechanisms may be valid hypotheses in another calibrated model, but they are not the standard physical model used by this project.

\subsection{Verification status}
The current-source implementation uses the exact Yuanhang float32 hysteresis path and keeps one thermal body and one device/domain. Deterministic current sweeps are vectorized across current amplitudes, and regression tests compare the vectorized output to serial traces sample by sample. Compatibility columns such as \texttt{g\_dyn} and \texttt{T\_sub} are retained only as aliases of the supported states.

At \(I=1640.226~\mu\mathrm{A}\), the representative Yuanhang current-source relaxation oscillation remains timestep-converged:
\begin{center}
\begin{tabular}{rrr}
\toprule
...
As a positive control, the original voltage-driven relaxation oscillator also remains oscillatory and converges as the timestep is reduced:
\begin{center}
\begin{tabular}{rrr}
\toprule
...
The project now has exactly two active physical simulation families: the original voltage-source circuit and the Yuanhang ideal-current-source reduction. All current-source claims should be made with the single supported implementation above and should continue to pass timestep and duration checks before being treated as physical.
