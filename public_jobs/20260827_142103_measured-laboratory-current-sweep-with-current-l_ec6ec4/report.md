# Measured laboratory current sweep with current-labeled onset bracket
...
The immediately preceding record is `250mv0_converted.csv` at **190 uA measured current step** (250 mV source setting). Its fixed 50--250 ns analysis window contains only **1 candidate peak**, so no periodic frequency is assigned.

The first coherently oscillating record is `300mv0_converted.csv` at **228 uA measured current step** (300 mV source setting). Its fixed 50--250 ns analysis window contains **8 peaks** with a period coefficient of variation of **2.03%**.
